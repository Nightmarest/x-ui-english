...
download_xui(){
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/Nightmarest/x-ui-english/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/') || last_version=$(curl -sm8 https://raw.githubusercontent.com/NidukaAkalanka/x-ui-english/main/config/version >/dev/null 2>&1)
    ...
    wget -N --no-check-certificate https://raw.githubusercontent.com/Nightmarest/x-ui-english/main/x-ui.sh -O /usr/bin/x-ui
